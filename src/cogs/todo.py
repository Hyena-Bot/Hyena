...
class Todo(commands.Cog):
    ...
    @commands.group(
        ...
        aliases=["to-do", "to_do"],
        usage="[p]todo",
        description="Manage your own todo list",
    )
    ...
    async def todo(self, ctx):
        if ctx.invoked_subcommand is None:
            ...
            embed.set_author(
                name="Hyena TO-DO system", icon_url=self.hyena.user.avatar.url
            )
            embed.add_field(
                name="Commands:",
                value="""
...
""",
            ...
            embed.add_field(
                name="Command usage:",
                value="`Usage [p]todo [command] [args]`",
                inline=False,
            )
            embed.set_footer(
    ...
    async def validate(self, ctx, todo: str) -> str:
        ...
        if todo.isnumeric():
            ...
            return await ctx.send(
                "Your todo is a numeric value, add more to the description"
            )
    ...
    async def add(self, ctx, *, todo: str):
        ...
        if not res or res[0]["todos"] == []:
            await self.db.execute(
                "INSERT INTO todo(user_id, todos) VALUES ($1, $2)",
                ctx.author.id,
                [todo],
            )
            await ctx.send(
                f"Successfully added `{todo}` to your todo list. I swear you're not gonna complete any of them. \n`Position: 1`"
            )
        if res:
            lst = res[0]["todos"]
            ...
            if len(lst) == 20:
                self.hyena.get_command("todo add").reset_cooldown(ctx)
                return await ctx.send(
                    "Your todo list has reached maximum limit [20] remove some of them [clear] to add more!"
                )

            await self.db.execute(
                "UPDATE todo SET todos = $1 WHERE user_id = $2",
                [*lst, todo],
                ctx.author.id,
            )
    ...
    async def list_todos(self, ctx):
        ...
        if res:
            ...
            home_embed.set_author(
                name=f"{ctx.author}'s ToDo(s)", icon_url=ctx.author.avatar.url
            )
...
""",
            ...
            for idx, x in enumerate(lst):
                ...
                embed.set_author(
                    name=f"Todo #{idx + 1}", icon_url=ctx.author.avatar.url
                )
            ...
            def check(reaction, user):
                return (
                    ...
                    and reaction.message.id == da_msg.id
                )
            ...
            while True:
                try:
                    ...
                    if da_reaction_he_made == "▶️":
                        ...
                        await da_msg.edit(
                            content="Stopped the todo list screen!", suppress=True
                        )
                        for emoji in pagination_emojis:
                            try:
                                await da_msg.clear_reaction(emoji)
    ...
    async def remove(self, ctx, position: str):
        ...
        if res:
            ...
            try:
                ...
                self.hyena.get_command("todo remove").reset_cooldown(ctx)
                return await ctx.send(
                    f"The todo position cannot be greater than your lenght of current todos: `{len(lst)}`"
                )

            await self.db.execute(
                "UPDATE todo SET todos = $1 WHERE user_id = $2", lst, ctx.author.id
            )
