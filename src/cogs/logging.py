import discord, random, asyncpg, asyncio, datetime
...
class Logging(commands.Cog):
    ...
    @property
    def category(self):
        return ["Utils", "Conf", "Mod"] # Choose from Utils, Mod, Fun, Conf ## Let it be in a list as we sometimes need to send two of these

    @commands.group(name="modlogs", aliases=['logs', 'mod_logs', 'log', 'logging'], usage="[p]logs", description="Logs for server members: Message logs, Leave/Join logs, and others.")
    async def logs(self, ctx):
        if ctx.invoked_subcommand is None:
            ...
            embed.set_author(name="Hyena Moderation Logs", icon_url=self.hyena.user.avatar.url)
...
"""
            embed.set_image(url="https://i.ibb.co/T40mXFh/Screenshot-2021-08-12-at-11-04-54-AM.png")
    ...
    async def channel(self, ctx, channel: discord.TextChannel):
        ...
        webhook = await channel.create_webhook(name="Hyena Logging", avatar=await self.hyena.user.avatar.with_format('png').read(), reason=f"Hyena Logging Configuration by {ctx.author}")
        ...
        result: asyncpg.Record = await self.db.fetch("SELECT * FROM logging WHERE guild_id = $1", ctx.guild.id)
        if not result:
            await self.db.execute("INSERT INTO logging(guild_id, channel_id, webhook_id) VALUES($1, $2, $3)", ctx.guild.id, channel.id, webhook.id)
            await imsg.edit(content="Injected to the database <:OP_Verified:815589801586851840>")
            await asyncio.sleep(0.5)
            await imsg.edit(content="Successfully finished the setup <:OP_Verified:815589801586851840>")
        if result:
            await imsg.edit(content="Found exisiting logs channel <:OP_Verified:815589801586851840>")
            await asyncio.sleep(0.2)
            try:
                oldwebhook = await self.hyena.fetch_webhook(result[0]['webhook_id'])

                await imsg.edit(content="Deleting old webhook <:OP_Verified:815589801586851840>")
                await asyncio.sleep(0.2)
                await oldwebhook.delete()
            except discord.NotFound:
                pass

            await self.db.execute("UPDATE logging SET webhook_id = $1, channel_id = $2 WHERE guild_id = $3", webhook.id, channel.id, ctx.guild.id)
            await imsg.edit(content="Updated the database <:OP_Verified:815589801586851840>")
            await asyncio.sleep(0.5)
            await imsg.edit(content="Successfully finished the setup <:OP_Verified:815589801586851840>")
    ...
    async def disable(self, ctx):
        result: asyncpg.Record = await self.db.fetch("SELECT * FROM logging WHERE guild_id = $1", ctx.guild.id)
        if not result:
            return await ctx.send("Logging is not setup for this guild <:NO:800323400449916939>")
        ...
        try:
            ...
            await imsg.edit(content="Deleting the old webhook <:OP_Verified:815589801586851840>")
            await oldwebhook.delete()
        except discord.NotFound:
        ...
        await imsg.edit(content="Deleting from the database <:OP_Verified:815589801586851840>")

        await imsg.edit(content="Successfully finished the removal <:OP_Verified:815589801586851840>")

    async def get_webhook(self, guild_id):
        result: asyncpg.Record = await self.db.fetch("SELECT * FROM logging WHERE guild_id = $1", guild_id)
        if not result:
            return None
        if result:
            try:
                ...
                channel = self.hyena.get_guild(guild_id).get_channel(result[0]['channel_id'])
                if not channel:
                    return None

                webhook = await channel.create_webhook(name="Hyena Logging", avatar=await self.hyena.user.avatar.url_as(format='png').read(), reason=f"Please do not delete these, I need them for logging.")
                await self.db.execute("UPDATE logging SET webhook_id = $1 WHERE guild_id = $2", webhook.id, guild_id)
                return webhook
            else:
                return webhook
    ...
    async def on_message_delete(self, message):
        ...
        embed = discord.Embed(color=self.colours['red'], timestamp=datetime.datetime.now())
        embed.set_author(name=message.author, icon_url=message.author.avatar.url)
        embed.title = f"Message deleted in {message.channel.name}"
        embed.description = f"**Message ID:** {message.id} \n**Message Content:** {message.content}"
        embed.set_footer(text=f"ID: {message.author.id}", icon_url=self.hyena.user.avatar.url)
    ...
    async def on_message_edit(self, before, after):
        ...
        embed = discord.Embed(color=self.colours['blue'], timestamp=datetime.datetime.now())
        ...
        embed.set_footer(text=f"ID: {before.author.id}", icon_url=self.hyena.user.avatar.url)
    ...
    async def on_guild_channel_create(self, channel):
        ...
        embed = discord.Embed(color=self.colours['green'], timestamp=datetime.datetime.now())
...
\n{created_at}"
    ...
    async def on_member_remove(self, member):
        ...
        embed = discord.Embed(color=self.colours['yellow'], timestamp=datetime.datetime.now())
        embed.set_author(name=member, icon_url=member.avatar.url)
...
\n**Roles [{len(roles)}]:** {_str}"
    ...
    async def on_user_update(self, before, after):
        if before == after:
            return
        webhooks = [await self.get_webhook(x.id) for x in after.mutual_guilds if await self.get_webhook(x.id) != None]
    ...
    async def on_bulk_message_delete(self, messages):
        ...
        embed = discord.Embed(color=self.colours['blue'], timestamp=datetime.datetime.now())
        embed.set_author(name=f"{len(messages)} messages purged in #{messages[0].channel.name}")
...
def setup(hyena):
    hyena.add_cog(Logging(hyena, {"red" : 0xe34d39, "blue" : 0x4287f5, "green" : 0x39e372, "yellow" : 0xe3c439}))