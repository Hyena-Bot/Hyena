...
class ServerConfig(commands.Cog):
    ...
    @commands.group(name="serverconf", aliases=['serverconfig', 'config', 'conf', 'server-config'], description="Adjust server-specific settings", usage="[p]serverconfig")
    @commands.cooldown(1, 3, commands.BucketType.user)
    async def serverconf(self, ctx):
        if ctx.invoked_subcommand is None:
            res = await self.hyena.tools.check_family_friendly(ctx, self.hyena)
            emoji = "<:OP_Verified:815589801586851840>" if res == True else "<:NO:800323400449916939>"
    ...
    @serverconf.command(name="family-friendly", aliases=['family', 'friendly', 'family_friendly'])
    ...
    async def family_friendly(self, ctx, value: str):
        ...
        res = await self.db.fetch("SELECT * FROM server_config WHERE guild_id = $1", ctx.guild.id)
        if res:
            await self.db.execute("UPDATE server_config SET family_friendly = $1 WHERE guild_id = $2", action, ctx.guild.id)
        if not res:
            await self.db.execute("INSERT INTO server_config(guild_id, family_friendly) VALUES($1, $2)", ctx.guild.id, action)
        await ctx.send(f"Successfully `{'enabled' if action else 'disabled'}` family friendly mode!")
