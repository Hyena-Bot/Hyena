...
class Moderation(commands.Cog):
    ...
    @commands.command(name="ban", usage="[p]ban [member] [delete_days] [reason : optional]", description="Yeet someone out of the server forever")
    @commands.has_permissions(ban_members=True)
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.bot_has_permissions(ban_members=True)
    async def ban(self, ctx, member: discord.Member, delete_days: Optional[int] = 0, *, reason="No reason provided"):
        if not delete_days in [0, 1, 7]:
            reason = f"{delete_days} " + reason
        try:
            if ctx.author.top_role > member.top_role or ctx.guild.owner == ctx.author and not member == ctx.author:
                try:
                    await member.send(f"**{ctx.guild.name}:** You have been 🔨 Banned \n**Reason:** {reason}")
                except:
                    pass

                await member.ban(delete_message_days=delete_days, reason=f"Banned by: {ctx.author}, Reason: {reason}.")
                ...
                if member == ctx.author:
                    ...
                    return await ctx.send("Error, this person has a higher or equal role to you")

        except discord.errors.Forbidden:
            return await ctx.send(f"Hmmm, I do not have permission to ban {member}.")

    @commands.command(usage="[p]kick [member] [reason : optional]", description="Kick someone out of this server :)")
    ...
    async def kick(self, ctx, member: discord.Member, *, reason="No reason provided"):
        try:
            if ctx.author.top_role > member.top_role or ctx.guild.owner == ctx.author and not member == ctx.author:
                try:
                    await member.send(f"**{ctx.guild.name}:** You have been 🦿 Kicked \n**Reason:** {reason}")
                except:
                    pass
    ...
    @commands.command(aliases=['revoke_ban', 'revoke-ban'], usage="[p]unban [member]", description="Revoke someone's ban")
    ...
    async def unban(self, ctx, member, *, reason="No reason provided"):
        ...
        if member.isnumeric():
            for ban_entry in bans:
                if ban_entry.user.id == int(member):
                    ...
                    await message.edit(f"🔓 Unbanned `{ban_entry.user}` \n**Reason:** {reason}")
                    success = True
        ...
        if success:
            try:
                await member.send(f"**{ctx.guild.name}:** You have been 🔓 unbanned \n**Reason:** {reason}")
            except:
                pass

            embed = discord.Embed(color=random.choice(self.hyena.colors))
            embed.set_author(name=f"UNBAN | {unbanned_user}", icon_url=unbanned_user.avatar.url)
            embed.add_field(name="User", value=f"{unbanned_user.name}")
            embed.add_field(name="Moderator", value=f"{ctx.author.name}")
            embed.set_footer(text=f"Moderator: {ctx.author}", icon_url=ctx.author.avatar.url)
            await self.logging.send(ctx, embed)
        if not success:
            await ctx.send(f"Cannot Find {member}, \nNOTE: You can send both IDs and their proper names whichever you like the most :)")

    @commands.command(aliases=['clear'], usage="[p]purge [amount]", description="Clear messages from a channel")
    @commands.cooldown(1, 3, commands.BucketType.user)
    ...
    async def purge(self, ctx, amount, member: discord.Member=None):
        ...
        if not amount.isnumeric():
            return await ctx.send("Only `Integers (Numbers), all, nuke` will be accepted")
        amount = int(amount)            
        ...
        if member is not None:
            purged_messages = await ctx.channel.purge(limit=amount, after=back, check=lambda x: not x.pinned and x.author.id == member.id)
            p = len(purged_messages)
            message = await ctx.send(f'Successfully purged `{p}` messages from `{member.name}` in the last `{amount}` messages!')
        else:
    ...
    @commands.command(name="lockchannel", aliases=['lock'], usage="[p]lock [channel : optional]", description="Lock a channel so that people can't talk in it")
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.check_any(commands.has_permissions(manage_channels=True), commands.is_owner())
    ...
    @commands.command(name="unlockchannel", aliases=['unlock'], usage="[p]unlock [channel : optional]", description="Unlock channel to give access to the people to talk in the channel")
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.check_any(commands.has_permissions(manage_channels=True), commands.is_owner())
    @commands.bot_has_permissions(manage_channels=True)
    async def unlock_channel(self, ctx, channel: discord.TextChannel = None):
        ...
        embed=discord.Embed(colour=random.choice(self.hyena.colours))
        embed.set_author(name=f"UNLOCK | {channel.name}", icon_url=ctx.author.avatar.url)
        embed.add_field(name="Channel", value=f"{channel.name}")
        embed.add_field(name="Moderator", value=f"{ctx.author.name}")
        embed.set_footer(text=f"Moderator: {ctx.author}", icon_url=ctx.author.avatar.url)
    ...
    @commands.command(name="nuke", aliases=["clone"], usage="[p]nuke [channel : optional]", description="Delete and clone a channel to clear all of the messages")
    ...
    async def nuke(self, ctx, channel:discord.TextChannel=None):
        ...
        if ctx.guild.id in blacklisted_guilds and not ctx.author.id in self.hyena.owner_ids:
            return await ctx.send("This command is disabled for this guild. This is managed directly by bot owners Donut#4427 and Div_100#5748. \n\nWhat can you do? Try and appeal this case in the support server: `https://discord.gg/cHYWdK5GNt` if the owner accepts it, it's good else you're done for. <:kek:826293846689185842>")
        if channel is None:
            channel = ctx.channel
        ...
        try:
            reaction, user = await self.hyena.wait_for("reaction_add", timeout=30, check=lambda reaction, user: user == ctx.author and str(reaction.emoji) == '✅' or '❌')
            ...
            if str(reaction.emoji) == '✅':
                ...
                try:
                    ...
                    position=position, topic=topic, slowmode_delay=slowmode,
                ...
                em.set_author(name=f"NUKE | {channel.mention}", icon_url=ctx.author.avatar.url)
                em.add_field(name="Channel", value=channel.name)
                em.add_field(name="Moderator", value=ctx.author)
                em.set_footer(text=f"Moderator: {ctx.author}", icon_url=self.hyena.user.avatar.url)

                self.logging.send(ctx, em)
            else:
                await ctx.send("K! I will not nuke this channel!")

    @commands.command(name="softban", description="Bans and immediately unbans a member to act as a purging kick.", usage="[p]softban [member] [delete_days : optional] [reason : optional]")
    @commands.check_any(commands.has_permissions(ban_members=True), commands.is_owner())
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.bot_has_permissions(ban_members=True)
    async def softban(self, ctx, member: discord.Member, delete_days: Optional[int] = 1, *, reason="No reason provided"):
        if not delete_days in [0, 1, 7]:
            reason = f"{delete_days} " + reason
        ...
        if ctx.author.top_role <= member.top_role and not ctx.author.id == ctx.guild.owner.id:
            return await ctx.send(f"Your top role `{ctx.author.top_role}` is smaller or equal to {member}'s top role `{member.top_role}")

        await ctx.guild.ban(member, reason="Softbanned by " + str(ctx.author), delete_message_days=delete_days)
        await ctx.guild.unban(member, reason="Softbanned by " + str(ctx.author))

        try:
            await member.send(f"**{ctx.guild.name}:** You have been 🔨 Softbanned \n**Reason:** {reason}")
        except:
            pass
