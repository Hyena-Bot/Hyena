...
class Giveaways(commands.Cog):
    ...
    async def giveaway(self, ctx):
        if ctx.invoked_subcommand is None:
            embed = discord.Embed(
                color=random.choice(self.colours), timestamp=ctx.message.created_at
            )
            embed.set_author(
                name="Hyena Giveaways", icon_url=self.hyena.user.avatar.url
            )
            embed.description = """
...
"""
    ...
    async def create(self, ctx):
        result__limit = await self.db.fetch(
        ...
        await ctx.send(
            "**Let's start this giveaway, answer the question within 30 seconds, Type `cancel` anytime to stop the setup!**"
        )
        questions = [
        ...
        for idx, question in enumerate(questions):
            ...
            try:
                ...
                return await ctx.send(
                    "You didn't answer in time. Timeout is `30` seconds"
                )
            else:
                ...
                if idx == 1:
                    try:
                        ...
                        return await ctx.send(
                            "You didnt properly mention a channel or give the ID"
                        )
                    else:
                        vals = [
                            ...
                            host_channel.permissions_for(
                                ctx.guild.me
                            ).read_message_history,
                        ]
                        if all(vals) == False:
                            return await ctx.send(
                                "I need the following permissions to make a giveaway: `Manage Messages`, `Send Messages`, `Add reactions`, `Send messages`, `Embed links`, `Read message history`"
                            )
                ...
                if idx == 2:
                    ...
                    if dur_secs in [-1, -2]:
                        return await ctx.send(
                            f"You didn't answer the question properly with the proper unit `[s|m|h|d]`"
                        )
                    if dur_secs > 604800:
                        return await ctx.send(
                            "Your giveaway duration cannot be greater than 7 days"
                        )
                ...
                if idx == 4:
                    if msg.content.lower() in ["no", "none", "no bruh", "me"]:
                        ...
                        continue
                    try:
                        mem = await commands.MemberConverter().convert(ctx, msg.content)
                ...
                if idx == 5:
                    ...
                    if isinstance(inv.guild, discord.invite.PartialInviteGuild):
                        return await ctx.send(
                            f"I cannot host this giveaway because I am not a member of `{inv}`"
                        )
                ...
                if idx == 6:
                    if str(msg.content.lower()) in ["no", "none", "no bruh"]:
                        ...
                        continue
                    try:
                        host_role = await commands.RoleConverter().convert(
                            ctx, msg.content
                        )
                    except commands.errors.RoleNotFound:
                        return await ctx.send(
                            "You didnt properly mention a role or give the ID"
                        )
                    else:
        ...
        guild_req = (
            f"\n> [Optional] Be in [{responses[5].guild.name}]({str(responses[5])})"
            if responses[5] != None
            else "\u200b"
        )
        role_req = (
            f"\n> Must have the {responses[6].mention} role"
            if responses[6] != None
            else "\u200b"
        )

        tasks = (
            f"\n\n**<:Tasks:846652002079211542> Tasks** {guild_req} {role_req}"
            if guild_req != "\u200b" or role_req != "\u200b"
            else "\u200b"
        )
        ...
        embed.set_author(
            name="Hyena Giveaways",
            icon_url="https://cdn.discordapp.com/emojis/813009441778958367.gif",
        )
        embed.description = f"""
**Prize: {prize}**
**React with 🎉 to enter!**
...
"""
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        result = await self.db.fetch(
            "SELECT * FROM giveaways WHERE guild_id = $1 AND msg_id = $2",
            guild.id,
            m.id,
        )
        if not result:
        ...
        if result:
            ...
            if done_reqs == True:
                ...
                embed.set_author(
                    ...
                    icon_url="https://cdn.discordapp.com/emojis/717382820363763852.png",
                )
                embed.description = (
                    f"Your entry for [this giveaway]({m.jump_url}) has been confirmed."
                )
                embed.set_footer(
                    text="Hyena Giveaway Confirmed", icon_url=self.hyena.user.avatar.url
                )
                try:
...
"""
                embed.set_footer(
                    text="Hyena Giveaway Denied Bruh",
    ...
    async def check_and_end(self, record):
        if int(record["end_time"]) < time.time():
            ...
            if guild is None:
                await self.db.execute(
                    "DELETE FROM giveaways WHERE guild_id = $1 AND msg_id = $2",
                    record["guild_id"],
                    record["msg_id"],
                )
                return None
            try:
                ...
                if not reaction:
                    ...
                    embed.set_footer(
                        text="Hyena Giveaway ended at: ",
                        icon_url=self.hyena.user.avatar.url,
                    )

                    await m.edit(
                        content=f"{self.emoji} **GIVEAWAY ENDED** {self.emoji}",
                        embed=embed,
                    )
                    await self.db.execute(
                        "DELETE FROM giveaways WHERE guild_id = $1 AND msg_id = $2",
                    ...
                    embed__2 = discord.Embed(
                        color=0x42B6F5, timestamp=datetime.datetime.now()
                    )
                    embed__2.set_author(
                        name="HYENA GIVEAWAY ENDED",
                        icon_url="https://media.discordapp.net/attachments/794467787988008976/847715051376607232/1f381.png",
                    )
                    embed__2.description = f"**Giveaway has ended!** \n**🎁 Gift •** {record['prize']} \n**🏅 Winners •** Nobody has won this Giveaway because nobody joined. \n[JUMP!]({m.jump_url})"
                    embed__2.set_footer(
                        text="Ended at: ", icon_url=self.hyena.user.avatar.url
                    )
                ...
                embed = discord.Embed(
                    ...
                    title="Hyena Giveaway Ended",
                )
                embed.add_field(
                    name="**🎁 Gift •**", value=record["prize"], inline=False
                )
                embed.add_field(name="**🏅 Winners •**", value=winners, inline=False)
                if has_req:
                    embed.add_field(
                        name="**<:role:847791584119881739> Role requirement •**",
                        value=role.mention,
                    )
                embed.set_footer(
                    text="Hyena Giveaway ended at: ",
                ...
                await m.edit(
                    content=f"{self.emoji} **GIVEAWAY ENDED** {self.emoji}", embed=embed
                )
    ...
    async def reroll(self, ctx, msg_id):
            ...
            if m.embeds != []:
                if str(m.embeds[0].footer) != "EmbedProxy()":
                    if str(m.embeds[0].footer.text) != "Embed.Empty":
                        if m.embeds[
                            0
                        ].footer.text == "Hyena Giveaway ended at:" and m.embeds[
                            0
                        ].title in [
                            "Hyena Giveaway Rerolled",
                            "Hyena Giveaway Ended",
                        ]:
                            is_giveaway = True

        if is_giveaway == False:
            return await ctx.send(
                "Either that message doesn't have any entrants while it ended or it's not a Hyena Giveaway."
            )
        ...
        if m.embeds[0].title == "Hyena Giveaway Rerolled":
            ...
            while 1:
                if nwins > len(users):
                    return await ctx.send(
                        f"There are not enough users to be rerolled. Max limit : {len(users)}"
                    )
                if nwins == len(users):
            ...
            new_embed.add_field(
                name="**🎗️ Old winners •**",
                value=", ".join([x.mention for x in old_mem_objs]),
                inline=False,
            )
            new_embed.add_field(
                name="**🏅 New winners •**", value=", ".join(winners), inline=False
            )
            if role_req is not None:
                new_embed.add_field(
                    name="**<:role:847791584119881739> Role requirement •**",
                    value=role_req.mention,
                )
            new_embed.set_footer(
                text="Hyena Giveaway ended at: ", icon_url=self.hyena.user.avatar.url
            )

            await m.edit(embed=new_embed)
            await m.channel.send(
                f"**Old Winners:** {', '.join([x.name for x in old_mem_objs])} \n**New Winners:** {', '.join(winners)}"
            )
        else:
            old_winners = [
                int(x)
    ...
    async def end(self, ctx, msg_id):
        ...
        if not record:
            return await ctx.send(
                "Cannot any giveaway like that in my database. Make sure that giveaway is not already ended or use `[p]giveaway reroll`"
            )
        else:
            record = record[0]
            reaction = None
            ...
            if not reaction:
                ...
                await m.channel.send(
                    content="Lmfao no one joined your giveaway so a winner cannot be chosen",
                    embed=embed__2,
                )
                await ctx.send(
                    "Lmfao no one joined your giveaway so a winner cannot be chosen"
                )
                return
    ...
    async def view(self, ctx):
        ...
        for record in records:
            ...
            if channel is None:
                self.db.execute(
                    "DELETE FROM giveaways WHERE guild_id = $1 and channel_id = $2",
                    ctx.guild.id,
                    record["channel_id"],
                )
                continue
