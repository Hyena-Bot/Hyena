import discord, json, random, string, sqlite3
...
class Warns(commands.Cog):
	...
	@commands.command(name="warn", description="Warn a user.", usage='[p]warn <member> [reason]')
	...
	async def warn(self, ctx, member: discord.Member, *, reason="None"):
		...
		if len(reason) > 150:
			return await ctx.send("Reason cannot be more than 150 characters. Optimization issues.")

		data = await self.db.fetchrow("SELECT * FROM warns WHERE guild_id = $1 AND user_id = $2", ctx.guild.id, member.id)
		id = "".join([random.choice(random.choice([string.ascii_letters, string.digits, string.hexdigits])) for i in range(10)])
		if data is None:
	...
	@commands.command(name='warns', aliases=['infractions'], description="View someone's infractions.", usage="[p]warns <member> [page]")
	@commands.check_any(commands.has_permissions(manage_messages=True), commands.is_owner())
	@commands.cooldown(1, 5, commands.BucketType.user)
	async def warns(self, ctx, member: discord.Member=None, page=1):
		...
		try:
			...
			return await ctx.send(f"Page needs to be an integer ( Number ) Got `{page}`.")
		page -= 1
		warns = await self.db.fetchrow("SELECT * FROM warns WHERE guild_id = $1 AND user_id = $2", ctx.guild.id, member.id)
		if warns is None:
			embed = discord.Embed(
				title="Warns",
		...
		try:
			...
			return await ctx.send("What are you even thinking bruh? That page does not exist.")

		embed = discord.Embed(
			title="Warns for the user.",
			color=random.choice(self.colours)
		).set_author(name=str(member), icon_url=(member.avatar.url or member.default_avatar.url))
		description = "\n".join([f"**ID: {e['id']}**\nReason: {e['reason']}" for e in warns])
		description = f"**Page {page+1}/{len(_data)}**" + "\n\n" + description
		embed.description = description
		await ctx.send(embed=embed)

	@commands.command(name="clear-warn", aliases=['clear_warn', 'remove-warn', 'remove_warn'], description="Remove a warn from a user.", usage='[p]clear-warn <member> [id]')
	@commands.check_any(commands.has_permissions(manage_messages=True), commands.is_owner())
	...
	async def clear_warn(self, ctx, member: discord.Member, id):
		...
		if id not in [e['id'] for e in data]:
			return await ctx.send(f"{member} has no warns with the ID `{id}`\n**Note: Capitalisation matters.**")
		_warn = None
		for warn in data:
			if warn['id'] == id:
				_warn = warn
		...
		await self.db.execute("UPDATE warns SET warn_data = $1 WHERE guild_id = $2 AND user_id = $3", data, ctx.guild.id, member.id)
	...
	@commands.command(name="clear-warns", aliases=['clear_warns', 'remove-warns', 'remove_warns'], description="Remove all warns from a user.", usage='[p]clear-warns <member>')
	@commands.check_any(commands.has_permissions(manage_messages=True), commands.is_owner())
	...
	async def clear_warns(self, ctx, member: discord.Member):
		...
		await self.db.execute("DELETE FROM warns WHERE guild_id = $1 AND user_id = $2", ctx.guild.id, member.id)
