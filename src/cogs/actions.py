...
class ModActions(commands.Cog):
    ...
    async def actions(self, ctx, user: discord.Member = None, page=1):
        ...
        page -= 1
        data = await self.db.fetchrow("SELECT * FROM moderation_actions WHERE guild_id = $1 AND user_id = $2", ctx.guild.id, user.id)
        if data is None:
            return await ctx.send(f"Uh oh, I couldn't any moderation actions ( done using hyena ) for `{user}`.")
        ...
        for data_block in data:
            xd = ', '.join([f"**{key.replace('_', ' ').title()}**: {data_block[key]}" for key in data_block.keys() if key not in ['time', 'moderator']]) + f"\n **Moderator**: {data_block['moderator']}, **Time**: {data_block['time']}"
            embed.add_field(name=f"{data_block['action']} for {member}", value=xd, inline=inline)
            inline = (not inline)
    ...
    async def clear_actions(self, ctx, member: discord.Member):
        await self.db.execute("DELETE FROM moderation_actions WHERE user_id = $1 AND guild_id = $2", member.id, ctx.guild.id)

        await ctx.send(f"Cleared all moderationm action LOGS for {member}. \n**Note: For stuff like warns it will not clear the actual warns, just the logs.**")
